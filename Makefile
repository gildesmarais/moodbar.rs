.PHONY: test test-core test-cli parity fmt lint check tdd tdd-core ci wasm publish-check-wasm wasm-docs native native-ios native-android publish-check-native publish-check-native-ios publish-check-native-android
...
build:
	cargo build -p moodbar

build-production:
	cargo build -p moodbar --release

install:
	cargo install --path crates/moodbar-cli --locked --force
